function toggleCharges() {
    let section = document.getElementById("chargesSection");
...
function calculateLoanEMI() {
    ...
    // Extra Charges
    let processingPercent = parseFloat(document.getElementById("processingFee").value) || 0; // %
    ...
    // Step 2: Processing fee (percentage of finance amount)
    let processingFee = financeAmount * (processingPercent / 100);
    ...
    document.getElementById("emiResult").innerHTML = `
        ...
        <p><b>Processing Fee (${processingPercent}%):</b> ₹${processingFee.toFixed(2)}</p>
        <p><b>Other Charges (Stamp + Extra + Owner):</b> ₹${(stampCharges + extraCharges + ownerCharges).toFixed(2)}</p>
